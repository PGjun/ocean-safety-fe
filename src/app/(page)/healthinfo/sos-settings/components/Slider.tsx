...
export const Slider = ({
  ...
  const calculateBackground = (value: any) => {
    ...
    const percentage = ((value - 40) / (80 - 40)) * 100
  ...
  return (
    <div className="flex w-full flex-col items-center border-b border-[#F1F3F5] py-[20px] md:border-none">
      <div className="flex w-full items-center justify-between gap-[24px]">
        <div className="w-[160px] text-[18px]">{title}</div>
        <input
          type="number"
          value={value}
          onChange={handleChange}
          className="border-#C4C4C4 flex-1 rounded-md border p-[16px] text-[14px] leading-[13.16px]"
        />
      </div>
